...
const Model = require('../../models/index').PartnerService;
...
export default ({ config, db, validate, validation }) => resource({
  ...
  create: [
    validate(validation.partnerServices.create),
    async ({ body, params }, res) => {
      try {
        const model = await Model.create({...body, serviceId: params.serviceId});
