...
module.exports = (res) => {
    ...
    /**
     * Returns a json positive response, which respects a pagination format.
     * Receives data and code
     *
     * @param data (string || object)
     * @param pageCount
     ...
     * @returns {*}
     */
    const respondPaginated = (data, pageCount, itemCount, req, code) => {
        return json({
            errors: null,
            data: {
                rows: data.rows,
    ...
    /**
     * Returns a response with the data collection, which respects a pagination format
     *
     * @param options
     * @param transformer (optional)
     * @param code (optional)
     * @returns {*}
     */
    const collectionPaginated = (options, transformer, code) => {
        return respondPaginated(
            transformer ? {
              ...options.data,
              rows: transformer(options.data.rows)
            } : options.data,
            options.pageCount || null,
            options.data ? (options.data.count || null) : null,
            options.req,
            code ? code : 200,
        );
    };
